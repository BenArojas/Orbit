...
- **TWS follow-up missions (new branches):** (1) **Live trading design gate** — live-account path, live safety confirmation, and port/account policy before any live code. (2) **Advanced order types** — brackets, scale-outs, stops/trailing, OCA, GTD/MOC/LOC/conditions. (3) **Market-data extras** — Level 2/depth panel, streaming quotes, entitlement detection beyond observed quote responses, and per-exchange subscription guidance; subscription/account-management automation stays out.
