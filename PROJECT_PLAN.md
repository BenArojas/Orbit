...
- **Cloud + Hybrid AI:** IN PROGRESS on `feature/orbit-v2-cloud-hybrid-ai-spec`; the AI Run Inspector/provider-controls work is code-complete, the prompt-grounding review-blocker remediation is now verified, and the manual OpenRouter smoke/merge gate still remains pending. Grounding now requires exact rendered fact IDs plus exact cent-matched prices from explicit price-bearing prompt facts, eval fixtures derive their allowed IDs/maps from the production fact pipeline, confidence-label compatibility is normalized only in production finalization, and telemetry comparisons fail closed when cost/latency are unavailable. Local Ollama stays the default, cloud is explicit opt-in, keys stay in the OS keychain, and SQLite stores only opaque `api_key_ref` values. Design: `docs/superpowers/specs/2026-06-05-orbit-v2-cloud-hybrid-ai-design.md`; plan: `docs/superpowers/plans/2026-06-19-ai-prompt-grounding-evaluation-loop.md`.
- **AI Analysis Data and Grounding Pipeline:** ACTIVE REMEDIATION on `feature/orbit-v2-cloud-hybrid-ai-spec`. Fixing incorrect intraday candle semantics (1H/4H using raw 1min/5min bars instead of true 1h/4h bars), silent EMA fact discard (ema_ prefix mismatch), insufficient history for EMA-200, and missing grounded price candidates for current close, EMA levels, BB bands, and VWAP. Three-slice plan: (1) correct candle semantics + EMA facts, (2) expose explicit grounded price candidates, (3) align prompt guidance. Plan: `docs/superpowers/plans/2026-06-21-ai-analysis-data-grounding-pipeline.md`.
