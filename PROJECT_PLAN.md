...
- **TWS Execution Assistant — Paper Order MVP: CODE COMPLETE on `feature/tws-execution-assistant-spec` (2026-06-27).** All five slices shipped: (1) fail-closed paper-only gate (ports 4002/7497; unknown/live ports read-only), exact order preview model (`PaperOrderPreview`, `paper_only=true, transmit=false`). (2) Backend paper submit via `TwsBrokerAdapter.place_paper_order()` — adapter rechecks all gates, returns TWS-reported broker status (`trade.orderStatus.status or "sent_to_tws"`), kill switch blocks placement. (3) Frontend confirm/submit loop — preview card → explicit "Place paper order" confirm button → duplicate-submit disabled while pending → Open Orders auto-refreshed on success. (4) Typed failure handling — gate rejections map to typed error codes via `TwsPlaceOrderGuardError` (order never sent, outcome deterministic); only failures at/after `placeOrder()` return `unknown_outcome` (409); `unknown_outcome` auto-refreshes reconciliation and status immediately so the user sees TWS state before retrying; manual "Refresh Open Orders" button available as backup. (5) Submission result card with broker order ID, TWS status, and "Refresh Open Orders" button; plan stays visible as submitted in local UI state. 19 backend tests pass; typecheck clean. **Deferred (requires separate design/safety gate before implementation):** cancel/modify/reply workflows; live trading path and live safety gate; DB-backed execution plans, audit ledger, and restart recovery; Level 2/depth and streaming quotes; bracket/trailing/OCA/GTD order types. Design: `docs/superpowers/specs/2026-06-05-tws-execution-assistant-design.md`. Plan: `docs/superpowers/plans/2026-06-27-tws-paper-order-mvp-plan.md`.
- **TWS Order Management — Slices 1+2+3 CODE COMPLETE on `feature/tws-execution-assistant-spec` (2026-06-28). HITL gate before Slice 4.** Slice 1: shared `TWS_ORDER_CAPABILITIES` contract (backend `tws_order_capabilities.py` + frontend `orderCapabilities.ts`); STP/STP LMT draft/validate/preview/submit; `stop_price` threaded through all models; `_apply_plan_prices()` sets `lmtPrice`/`auxPrice`. Slice 2: `DELETE /execution-assistant/orders/{order_id}` cancel endpoint; `_ensure_paper_order_mutation_allowed()` adapter guard; Cancel button on every Open Orders row. Slice 3: `PATCH /execution-assistant/orders/{order_id}` modify endpoint; `modify_order()` adapter method; Modify button for LMT/STP/STP LMT rows; Execution Plan panel switches to edit mode (modify form → before/after review → submit); unsupported types (MKT) show disabled. 26 backend tests pass; typecheck clean. Manual smoke (cancel + modify open orders) pending human. No DB persistence. No live trading. Slice 4 (advanced overrides) standalone. Design: `docs/superpowers/specs/2026-06-28-tws-order-management-design.md`. Plan: `docs/superpowers/plans/2026-06-28-tws-order-management-implementation.md`.
- **TWS Broker Cockpit UI:** CODE COMPLETE on `feature/tws-broker-cockpit-ui`. UI-only redesign of the disconnected TWS page from the approved Broker Cockpit mockup: full-width cockpit shell, connection/reconciliation/draft gate strip, locked execution-plan panel before reconciliation, right-side safety rail, and empty positions/open-orders tables. Verified with `npm run typecheck` and `npm run build`; browser smoke was blocked by the existing localhost FastAPI CORS preflight behavior in dev. No backend, adapter, arming, submission, or safety-policy behavior changes. Design: `docs/superpowers/specs/2026-06-27-tws-broker-cockpit-ui-design.md`.
- **TWS Market Data Capability — COMPLETE on `feature/tws-broker-cockpit-ui` (2026-06-27):** All five slices shipped. (1) `QuoteSnapshot` extended with `market_data_type`, `is_delayed`, `unavailable_reason`, `error_code`; `get_quote()` calls `reqMarketDataType(4)` for delayed-frozen fallback; data-first classification ensures delayed data returned alongside error 10089 is not discarded. (2) TypeScript `QuoteSnapshot` mirrored; quote strip shows all 7 price fields, data-type badge (live/frozen/delayed), and `unavailable_reason`. (3) Always-visible inline guidance per `market_data_type`/`error_code`; copy approved by user (HITL gate satisfied in session). (4) Session Health "Market data" row reflects last quote state. (5) `_SuppressExpectedMdtWarnings` log filter (word-boundary regex, scoped to `get_quote()` via `finally`) downgrades 10089/10090 ib_async lines to DEBUG; unexpected errors still log. Verified: 7 backend tests pass, `npm run typecheck` clean. Reviewer verdict: ready with minor follow-ups addressed. Plan: `docs/superpowers/plans/2026-06-27-tws-market-data-capability-plan.md`.
- **TWS Terminal Cockpit Redesign — CODE COMPLETE on `feature/tws-execution-assistant-spec` (2026-06-28):** All four slices shipped. (1) Workflow compression: draft + validate + preview chained into a single `Review order` action; `Place order` remains the explicit confirm; submitted receipt shows broker order ID and TWS status. (2) Light-mode audit: module-only contrast issues fixed using existing Orbit `.light`/`.dark` CSS variable tokens. (3) Terminal cockpit layout: compact status strip (reconciled, position count, open-order count, kill switch, TWS-only badge); stateful execution plan panel (ticket → preview → submitted); right-side chart + quote panel; Positions and Open Orders side-by-side at the bottom with `max-h-[160px] overflow-y-auto`. (4) Read-only TWS bar data: `GET /execution-assistant/instruments/{conid}/bars?timeframe=` endpoint using `ib_async.reqHistoricalDataAsync`; `TwsCandleChart` component using TradingView Lightweight Charts v5 with candlestick + volume series, dashed cyan crosshair, no TV watermark. UI polish beyond spec: quote strip moved above chart as a compact always-visible horizontal bar (Last/Close/High/Low with animated skeletons while loading, exchange label, Delayed/Live badge); IBKR -1 sentinel filtered so suppressed fields never render; all panel backgrounds shifted to match dark mockup tokens; chart and plan panels share equal responsive height; ticket and preview panels use `justify-between` so action buttons stay at the bottom. Verified: `npm run typecheck` clean. Pending: human smoke test and merge. Design: `docs/superpowers/specs/2026-06-28-tws-terminal-cockpit-redesign.md`.
