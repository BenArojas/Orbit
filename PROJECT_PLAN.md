...
- **Cloud + Hybrid AI:** IN PROGRESS on `feature/orbit-v2-cloud-hybrid-ai-spec`; the AI Run Inspector/provider-controls work is code-complete, the approved prompt-grounding tracer bullet now starts from `docs/superpowers/plans/2026-06-19-ai-prompt-grounding-evaluation-loop.md`, and the manual OpenRouter smoke/merge gate still remains pending. Local Ollama stays the default, cloud is explicit opt-in, keys stay in the OS keychain, and SQLite stores only opaque `api_key_ref` values. Design: `docs/superpowers/specs/2026-06-05-orbit-v2-cloud-hybrid-ai-design.md`.
