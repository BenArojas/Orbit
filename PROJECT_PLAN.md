...
| Architecture follow-up — Instrument Identity module | SLICE 2 COMPLETE on `feature/instrument-identity-module` | Quote, search, and `/market/conid/{symbol}` display-identity writes now route through `InstrumentIdentityService`; `/instruments/{conid}` reads through the service with typed cache-miss behavior; `useInstrument()` consumes the unchanged API shape. Remaining slices: watchlist normalization and Inflect fallback. Spec: `docs/superpowers/specs/2026-06-06-instrument-identity-module-design.md`. Verified with focused backend route/conid-cache tests, hook test, and `tsc --noEmit`. |
