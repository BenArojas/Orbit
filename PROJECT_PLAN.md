...
- **TWS follow-up missions (design branch `feature/tws-live-advanced-market-data-design`):** parent decision-locking spec approved in `docs/superpowers/specs/2026-06-29-tws-live-advanced-market-data-design.md`. Mission order is (1) **Live trading** — live place/cancel/modify after explicit allowlisted session arming. (2) **Advanced order types** — long-only scale-out ladders first, plus brackets, trailing stops, OCA, GTD/MOC/LOC, and conditions. (3) **Market-data extras** — entitlement diagnostics first, then TWS-owned websocket streaming for live quotes/candles, Level 2/depth, and per-exchange guidance; subscription/account-management automation stays out.
