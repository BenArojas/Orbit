...
| 4.9 | AI panel — chat interface | Ben | DONE | AiChatPanel (orchestrator), AiSetupGuide (inline install/model guidance for all Ollama states), AiModelSelector (dropdown from installed models), useAiStatus (TanStack Query polling), useAiStream (SSE fetch+ReadableStream). Zustand useAiStore for session, messages, signal, streaming state. AnalysisPage wired to real API. Branch: feature/ai-chat-ui |
| 4.10 | Ollama integration service (`services/ai.py`) | Ben | DONE | Gemma 4 26B A4B via Ollama. Structured JSON input (pre-computed indicator signals). AiService class with chat sessions, signal parsing, multi-timeframe analysis orchestration. Streaming support via SSE. Model passed per-request (not hardcoded). LRU session eviction (max 50). |
| 4.11 | AI analysis router (`routers/ai.py`) | Ben | DONE | 8 endpoints: POST /ai/analyze, POST /ai/chat, POST /ai/chat/stream, GET /ai/status, GET /ai/models, POST /ai/models/select, GET /ai/setup-guide, POST /ai/refresh. AI_INDICATOR_MAP resolves display names to backend names. |
| 4.12 | Ollama lifecycle management (detect/start) | Ben | DONE | `services/ollama.py` — detect-only philosophy. Checks 7 binary paths, starts server if found, lists installed models. Never downloads or installs. Guides user with setup guide + recommended models. Model selection persisted to SQLite. |
